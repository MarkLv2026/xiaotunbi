...
# 总转化率（总成交订单量/点击数）的同比
promo_yoy_torders = sum(r.get('_总成交订单量', 0) for r in promo_yoy)
promo_yoy_tcvr = promo_yoy_torders / promo_yoy_clicks * 100 if promo_yoy_clicks else 0
...
# 总转化率（总成交订单量/点击数）的环比
promo_mom_torders = sum(r.get('_总成交订单量', 0) for r in promo_prev)
promo_mom_tcvr = promo_mom_torders / promo_mom_clicks * 100 if promo_mom_clicks else 0
...
# ═══════════════════════════════════════════════════════════════
with tabs[0]:
    # ── 恢复 session_state 缓存（模块级不访问 st.session_state，避免启动崩溃）──
    _restore_from_session()
    
    # ── 懒加载触发：首次访问时自动解析数据（避免启动超时）──
    _need_rerun = False
